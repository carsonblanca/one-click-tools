...
export function ColorCardImage({ record }: { record: CatalogRecord }) {
  ...
  return (
    <div
      className="group/color-image relative z-0 aspect-square w-[180px] max-w-full lg:hover:z-30"
      data-color-image-slot
      data-product-key={productKey}
      data-official-color-code={officialColorCode}
    >
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="flex h-full w-full items-center justify-center overflow-hidden rounded-2xl transition-transform duration-200 ease-out lg:group-hover/color-image:scale-[1.25] 2xl:group-hover/color-image:scale-[1.15]" data-color-image-frame>
          {available ? (

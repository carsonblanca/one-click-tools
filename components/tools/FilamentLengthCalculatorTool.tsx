...
export default function FilamentLengthCalculatorTool({ locale = "en" }: { locale?: string }) {
  ...
  const calculate = () => {
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Could not calculate filament length.");
    }
  };
  ...
  return (
    <ToolPanel>
      <div className="grid gap-5 md:grid-cols-3">
        ...
        <div>
          <ToolLabel>Weight (grams)</ToolLabel>
          <ToolInput value={weight} onChange={setWeight} type="number" placeholder="1000" />
        </div>
      </div>
      ...
      {error ? (
        ...
        <>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <ToolStatCard label="Approximate filament length" value={formatMeters(lengthMeters)} />
            <ToolStatCard label="Diameter" value={`${diameter} mm`} />
          </div>
          <ToolResultBox muted>
            Estimate only. Actual length can vary with filament ovality, manufacturer density, humidity, additives, and measurement tolerances.
          </ToolResultBox>
        </>
      ) : (
        <ToolResultBox muted>Approximate filament length will appear here.</ToolResultBox>
      )}
    </ToolPanel>
  );
}

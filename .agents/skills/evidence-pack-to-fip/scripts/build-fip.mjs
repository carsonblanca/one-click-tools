...
function buildFip(inputPath, outputPath) {
  ...
  if (materialType) {
    identityCandidates.push({
      fieldCandidate: "材料类型",
      value: materialType,
      sourceText: `产品页面标注: ${materialType}`,
      sourceMethod: "official_product_page",
      reviewStatus: "pending",
    });
  }
  ...
  // v3-lite: parameter-candidates.json holds ONLY canonical-parameter candidates
  // generated by OCR at build time. Identity values (material type, diameter)
  // are captured on the product object itself and are never invented as new
  // candidate fields. review-only / official_product_page candidates from
  // parameter-evidence.json are preserved as canonical candidates when their
  // field is in the allowlist; otherwise they are intentionally dropped.
  const evidenceCandidates = parameterCandidates
    .filter((c) => CANONICAL_PARAMETER_FIELDS.includes(stringValue(c.fieldCandidate)))
    .map((c) => ({
      candidateId: stableHash([
        productLine || "",
        stringValue(c.fieldCandidate),
        stringValue(c.value) || rawStringValue(c.sourceText),
        "",
      ]),
      canonicalKey: stringValue(c.fieldCandidate),
      rawValue: stringValue(c.value),
      normalizedValue: stringValue(c.value),
      unit: "",
      ...
      source: {
        ocrTextPath: "",
        sourceImage: "",
        snippet: rawStringValue(c.sourceText),
      },
...
function verifyFip(bytes, expectations) {
  ...
  for (let index = 0; index < parameters.length; index += 1) {
    ...
    if (!stringValue(src.sourceImage)) fail(`Parameter candidate ${index} has no source image`);
    if (!stringValue(src.ocrTextPath)) fail(`Parameter candidate ${index} has no OCR text path`);

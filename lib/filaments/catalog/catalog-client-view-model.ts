...
export function hasPresetParameters(record: CatalogRecord) {
  if (record.presetDownloadAllowed === false) return false;
  if (record.brand === "R3D") return false;
  if (record.brand === "Kexcelled" && record.parameterStatus === "missing") return false;
  if (record.brand === "ALIZ" && record.parameterStatus === "missing") return false;
  if (record.brand === "MOCHUANG" && record.parameterStatus === "missing") return false;
  return true;
}

...
type DraftItem = { sourceRunId: string; name: string; published: boolean };
...
export default function DraftPublishControls({ drafts }: { drafts: DraftItem[] }) {
  ...
  async function publish(sourceRunIds: string[]) {
    if (!sourceRunIds.length || busy) return;
    setBusy(true);
    setMessage(null);
    try {
      const response = await fetch("/api/admin/filament-drafts/batch-publish", {
        ...
        body: JSON.stringify({ sourceRunIds }),
      });
    ...
    <div className="space-y-3">
      {pending.length ? (
        <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-slate-700">
          {pending.map((item) => (
            <label key={item.sourceRunId} className="inline-flex items-center gap-2">
              <input
                type="checkbox"
                checked={selected.includes(item.sourceRunId)}
                disabled={busy}
                onChange={(event) => {
                  setSelected((current) => event.target.checked
                    ? [...current, item.sourceRunId]
                    : current.filter((sourceRunId) => sourceRunId !== item.sourceRunId));
                }}
              />
              <span>{item.name}</span>
            </label>
      ...
      <button
        ...
        onClick={() => setSelected(selected.length === pending.length ? [] : pending.map((item) => item.sourceRunId))}
      >

...
export async function generateMetadata({
  ...
  const tool = getLocalized3dTool("zh-cn", slug);
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/zh-cn/tools/${tool.slug}`,
      languages: {
...
export default async function SimplifiedChineseToolPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return <LocalizedToolPageContent locale="zh-cn" tool={tool} />;
}
